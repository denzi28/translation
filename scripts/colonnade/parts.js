/**
 * Geometry and materials shared by every render: the Ionic column (attic
 * base, fluted shaft, bead-and-reel, egg-and-dart, volutes, abacus) and the
 * bronze lantern. Units: the lower diameter of the shaft is 1.
 */
import * as THREE from "three";

// ---------------------------------------------------------------- dimensions
// Units: the lower diameter of the shaft is 1.
export const R = 0.5;              // shaft radius
export const FLUTES = 24;
export const BASE_TOP = 0.43;      // where the attic base hands over to the shaft
export const SHAFT_TOP = 14.0;     // top of the shaft (astragal starts here)
export const LANTERN_Y = 7.0;      // height of the bracket arm
export const LAMP_X = 1.0;        // where the lantern hangs
export const XL = -1.0, XR = 1.3; // horizontal frame shared by every piece

// ------------------------------------------------------------------ helpers
export function rand(seed) { let s = seed >>> 0; return () => ((s = (s * 1664525 + 1013904223) >>> 0) / 4294967296); }
...
export function streak(theta) { let s = 0, n = 0; for (const t of streakTerms) { s += t.a * Math.sin(t.f * theta + t.p); n += t.a; } return s / n; }

export function setColors(geo, fn) {
...
export function tintFor(x, z, cav) { const s = streak(Math.atan2(z, x)); const k = cav * (1 + 0.035 * s); return [k * (1 - 0.004 * s), k, k * (1 + 0.006 * s)]; }
...
export function shaftSegment(y0, y1, roundBottom, roundTop) {
  const rowsY = [];
...
export function lathe(profile, seg = 256) {
  const g = new THREE.LatheGeometry(profile.map(([r, y]) => new THREE.Vector2(r, y)), seg);
  setColors(g, (x, y, z) => tintFor(x, z, 1));
  return g;
}
export function profileFn(fn, y0, y1, n) { const pts = []; for (let i = 0; i <= n; i++) { const t = i / n; pts.push([fn(t), y0 + (y1 - y0) * t]); } return pts; }

/** Square moulding (plinth, abacus): profile [[halfWidth, y], ...] swept round a square. */
export function squareMoulding(profile) {
...
// ------------------------------------------------------------- materials
export function marbleMaterial(night) {
...
// ---------------------------------------------------------------- column
/**
 * Adds a column to `scene`. `shaftTop` sets its height; without `base` the
 * shaft simply starts at `shaftBottom`, for columns cut off below the view.
 */
export function buildColumn(scene, night, { shaftTop = SHAFT_TOP, base = true, shaftBottom = BASE_TOP, mat = marbleMaterial(night) } = {}) {
  ...
  if (base) {
    ...
    add(shaftSegment(2.2, shaftTop - 2.2, false, false));
    add(shaftSegment(shaftTop - 2.2, shaftTop, false, true));
  } else {
    add(shaftSegment(shaftBottom, shaftTop - 2.2, false, false));
    add(shaftSegment(shaftTop - 2.2, shaftTop, false, true));
  }
...
// ---------------------------------------------------------------- lantern
export function lanternMaterials(night) {
  return {
    bronze: new THREE.MeshStandardMaterial({ color: 0x7a5530, metalness: 1, roughness: 0.38, envMapIntensity: night ? 0.35 : 1.0 }),
    darkBronze: new THREE.MeshStandardMaterial({ color: 0x4a3220, metalness: 1, roughness: 0.45, envMapIntensity: night ? 0.3 : 0.9 }),
    glass: night
...
/**
 * The chain and the lantern hanging from a hook at (x, hookY, z), drawn at
 * unit scale and enlarged by `S` about the hook. Returns the height of the
 * middle of the glass, where the flame is.
 */
export function lanternBody(scene, night, { x, hookY, z = 0, S = 1.35, links = 4, mats = lanternMaterials(night) }) {
  const { bronze, darkBronze, glass } = mats;
  const made = [];
  const add = (geo, m, cast = true) => { const mesh = new THREE.Mesh(geo, m); mesh.castShadow = cast; mesh.receiveShadow = true; scene.add(mesh); made.push(mesh); return mesh; };
  let cy = hookY;
  for (let i = 0; i < links; i++) {
    const link = new THREE.TorusGeometry(0.018, 0.005, 10, 24); link.scale(1, 1.5, 1);
    if (i % 2) link.rotateY(Math.PI / 2);
    link.translate(x, cy - 0.022, z); add(link, darkBronze); cy -= 0.042;
  }
  const top = cy - 0.02;
  const ring = new THREE.TorusGeometry(0.022, 0.006, 12, 32); ring.translate(x, top + 0.012, z); add(ring, bronze);
  const finial = new THREE.SphereGeometry(0.02, 24, 12); finial.translate(x, top - 0.01, z); add(finial, bronze);
  const roofPts = [[0.0, 0.0], [0.02, -0.005], [0.05, -0.03], [0.1, -0.07], [0.135, -0.1], [0.14, -0.108], [0.0, -0.108]].map(([r, h]) => new THREE.Vector2(r, h)).reverse();
  const roof = new THREE.LatheGeometry(roofPts, 8); roof.translate(x, top - 0.02, z); add(roof, bronze);
  const bandTop = top - 0.13;
  const b1 = new THREE.CylinderGeometry(0.128, 0.128, 0.022, 8); b1.translate(x, bandTop, z); add(b1, darkBronze);
  const gH = 0.22, gY = bandTop - 0.011 - gH / 2;
  const g = new THREE.CylinderGeometry(0.112, 0.112, gH, 8, 1, false); g.translate(x, gY, z); add(g, glass, !night);
  for (let i = 0; i < 8; i++) {
    const a = (i / 8) * Math.PI * 2 + Math.PI / 8;
    const bar = new THREE.CylinderGeometry(0.008, 0.008, gH, 8); bar.translate(x + 0.121 * Math.cos(a), gY, z + 0.121 * Math.sin(a)); add(bar, bronze);
  }
  const b2 = new THREE.CylinderGeometry(0.13, 0.12, 0.024, 8); b2.translate(x, gY - gH / 2 - 0.012, z); add(b2, darkBronze);
  const bottomPts = [[0.12, 0], [0.1, -0.03], [0.05, -0.06], [0.02, -0.075], [0, -0.08]].map(([r, h]) => new THREE.Vector2(r, h));
  const bottom = new THREE.LatheGeometry(bottomPts, 8); bottom.translate(x, gY - gH / 2 - 0.024, z); add(bottom, bronze);
  const drop = new THREE.SphereGeometry(0.018, 24, 12); drop.translate(x, gY - gH / 2 - 0.11, z); add(drop, bronze);
  if (night) {
    const flame = new THREE.SphereGeometry(0.03, 24, 16); flame.scale(1, 1.9, 1); flame.translate(x, gY - 0.02, z);
    add(flame, new THREE.MeshBasicMaterial({ color: 0xfff1c9 }), false);
  }
  for (const m of made) {
    m.geometry.translate(-x, -hookY, -z); m.geometry.scale(S, S, S); m.geometry.translate(x, hookY, z);
  }
  return { glassY: hookY + (gY - hookY) * S, meshes: made };
}

/** The column's lantern: collars round the shaft, a bracket, and the lantern. */
export function buildLantern(scene, night) {
  const mats = lanternMaterials(night);
  const { bronze, darkBronze } = mats;
  ...
  return lanternBody(scene, night, { x: LAMP_X, hookY: y + 0.03, mats });
}
